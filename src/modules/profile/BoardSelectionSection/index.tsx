...
export const BoardSelectionSection = ({ boards }: IProps) => {
  ...
  return (
    <>
      <CreateBoardModal />
      <div className="px-8">
        <h2 className="my-4 text-2xl font-bold">Your boards:</h2>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {boards.map((board) => (
            <BoardCard
              key={board.slug}
              content={board.slug}
              action={() => router.push(`/board/${board.slug}`)}
            />
          ))}
          <div className="tooltip tooltip-primary" data-tip="Add new board">
            <label htmlFor={"create-board-modal"}>
              <BoardCard
                content={
                  <svg
                    width="64"
